...
function removeFirstDuplicateQr(){
 var labels=Array.from(document.querySelectorAll('body *')).filter(function(el){return el.children.length===0 && /segui la storia nella mostra/i.test((el.textContent||'').trim())});
 var blocks=[];
 labels.forEach(function(label){var p=label.parentElement,depth=0;while(p&&p!==document.body&&depth<5){if(p.querySelector('img[src*="qr" i],img[alt*="qr" i],[class*="qr" i],[id*="qr" i]')){if(blocks.indexOf(p)<0)blocks.push(p);break}p=p.parentElement;depth++}});
 if(blocks.length>=2){blocks[0].remove();return}
 var imgs=Array.from(document.images).filter(function(img){return /qr/i.test((img.src||'')+' '+(img.alt||''))});
 if(imgs.length>=2){var first=imgs[0],p=first.parentElement,depth=0;while(p&&p!==document.body&&depth<4&&p.children.length<=3){if(/segui la storia nella mostra/i.test(p.textContent||''))break;p=p.parentElement;depth++}if(p&&p!==document.body)p.remove();else first.remove()}
}
...
function run(){css();menu();removeFirstDuplicateQr()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();window.addEventListener('load',function(){setTimeout(run,100);setTimeout(removeFirstDuplicateQr,700)});
})();
...
(function(){
  ...
  function setupCard(card){
    ...
    function setSpec(){
      ...
      b.textContent='Formato standard: 50 × 70 cm';
      specs.appendChild(a);
      specs.appendChild(b);
  ...
  function fixLntdvHeader(){
    /* Remove the stray literal backslash-n text node rendered above the header. */
    Array.from(document.body.childNodes).forEach(function(node){
      if(node.nodeType===3 && node.textContent.trim()==='\\n') node.remove();
    });

    /* Put the hamburger inside the header, pinned to its upper-right corner. */
    var menu=document.getElementById('infoMenuButton');
    ...
    /* Move the QR out of the header and place one real QR block after the complete project description. */
    var oldQr=document.getElementById('lntdv-story-qr-block');
    if(oldQr) oldQr.remove();
    var description=document.querySelector('.lntdv-title-description');
    if(description){
      var qr=document.createElement('div');
      qr.id='lntdv-story-qr-block';
      ...
      img.style.cssText='display:block;width:136px;height:136px;max-width:136px;object-fit:contain;margin:0 auto 8px;border:4px solid #fff;border-radius:4px;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.18);box-sizing:border-box;';
  ...
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',fixLntdvHeader);
  else fixLntdvHeader();

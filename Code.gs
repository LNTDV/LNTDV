...
function doPost(e) {
  try {
    ...
    sheet.appendRow([
      new Date(), orderId, customer.name || '', customer.street || '',
      customer.zip || '', customer.city || '', customer.email || '', itemText,
      subtotal, shipping, total, deliveryType, customer.note || '', false, 'NUOVO'
    ]);
...
function orderWindow_(orderId, key) {
  ...
  const html = `<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Gestione ${safe(orderId)}</title><style>body{font-family:Arial,sans-serif;background:#f3eadc;color:#3d281d;margin:0;padding:24px}.box{max-width:760px;margin:auto;background:#fffaf3;border:1px solid #d8c5ae;border-radius:16px;padding:24px;box-shadow:0 8px 30px #0001}h1{margin-top:0;color:#5a3b2b}.row{padding:10px 0;border-bottom:1px solid #eadbc9}.status{font-size:18px;font-weight:700;margin:22px 0}button{background:#5a3b2b;color:#fff;border:0;border-radius:9px;padding:12px 18px;font-size:16px;cursor:pointer;margin-right:8px;margin-top:8px}</style></head><body><div class="box"><h1>Gestione ordine ${safe(orderId)}</h1><div class="row"><b>Cliente:</b> ${safe(row[2])}</div><div class="row"><b>Email:</b> ${safe(row[6])}</div><div class="row"><b>Indirizzo:</b> ${safe(row[3])}, ${safe(row[4])} ${safe(row[5])}</div><div class="row"><b>Ordine:</b><br>${orderHtml}</div><div class="row"><b>Subtotale:</b> €${Number(row[8]||0).toFixed(2)}</div><div class="row"><b>Spedizione:</b> €${Number(row[9]||0).toFixed(2)}</div><div class="row"><b>Totale:</b> €${Number(row[10]||0).toFixed(2)}</div><div class="row"><b>Modalità:</b> ${safe(row[11])}</div><div class="row"><b>Note:</b> ${safe(row[12])}</div><div class="status">${received ? '✓ ORDINE RICEVUTO / PRESO IN CARICO' : '☐ ORDINE DA PRENDERE IN CARICO'}<br><small>Stato: ${safe(status)}</small></div><button onclick="setReceived(true)">✓ Segna ricevuto</button><button onclick="setReceived(false)">↶ Riapri ordine</button></div><script>function setReceived(v){google.script.run.withSuccessHandler(function(){location.reload()}).withFailureHandler(function(e){alert(e.message||e)}).setOrderReceived('${safe(orderId)}',v)}</script></body></html>`;
...
function getSettings_() {
  ...
  return {
    ...
    pickupText: String(out.pickupText || 'Ritiro da concordare a Milano'),
